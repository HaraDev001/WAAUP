  ...
  methods: {
    ...
    ShowSSFNumDLG: function ShowSSFNumDLG() {
      if (permss.pcreate == 1) {
        ...
        $('#myAlertNumAddModal #ssfn-fnumber').val('');
    ...
    getSFNumList: function getSFNumList() {
      ...
      axios__WEBPACK_IMPORTED_MODULE_0___default.a.post('admin.getSiteNumlist', postData).then(function (response) {
        ...
        if (msg == "ok" && total > 0) {
          ...
          pnum_n = pstart_n <= numg_n ? 1 : parseInt(pstart_n) - 1; // console.log("total:"+total);
          ...
          $('select[id^="ssfnumselect_"]').change(function () {
            ...
            if (sel_val != '') {
              if (sel_val == 'change') {
                ...
                if (permss.pdelete == 1) {
                  _assets_jsfunc_mjs_module__WEBPACK_IMPORTED_MODULE_1__["default"].data.confirmAlert(deleteSiteNumber, t_id, 'pgtxt_wantdeleteitem');
                } else _assets_jsfunc_mjs_module__WEBPACK_IMPORTED_MODULE_1__["default"].data.showAlert('pgtxt_notpermission');
              }
            }
          });
